...
nu = 1
lagrange_mult = True
hmax = 2**(-np.arange(0,index,dtype="float"))
...
#0: P2P0 h
[ov, oq, ob, conti] = elements[2]

for i in range(index):
    mesh = MakeGeometry(hmax[i])
    V = VectorH1(mesh, order=ov, dirichlet="wall")
    V.SetOrder(TRIG,ob)
    if conti==1:
        Q = H1(mesh, order=oq)
    else:
        Q = L2(mesh, order=oq,dgjumps=True)
    if lagrange_mult:
        N = NumberSpace(mesh)
        X = FESpace([V,Q,N])
    else:
    ...
    ##Solve System
    gfu, sol = SolveStokes(X,nu,lagrange_mult,1e-6)
